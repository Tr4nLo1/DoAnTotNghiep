                ...
                <div class="col-md-7 col-lg-8"> 
                    <div class="p-t-7 p-r-85 p-r-15-lg p-r-0-md"> 
                        <form action="" method="post"> @include('admin.alert')
                             <h4 class="mtext-105 cl2 txt-center p-b-30"> Chỉnh sửa thông tin </h4> 
                              @error('name') 
                             ...
                             <div class="bor8 m-b-20 how-pos4-parent"> 
                                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" type="text" value="{{$users->name}}" name="name" placeholder="Your Name"> 
                            </div> 
                            @error('email') 
                            <div class="alert alert-danger">{{ $message }}</div> 
                            @enderror <div class="bor8 m-b-30"> 
                                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" type="email" value="{{$users->email}}" name="email" placeholder="Your Email"> 
                            </div>
                            @error('password') 
                            <div class="alert alert-danger">{{ $message }}</div> 
                            @enderror 
                            ...
                            <div class="bor8 m-b-30"> 
                                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" type="numberphone" value="0{{$users->phone}}" name="phone" placeholder="Your Phone"> </div> 
                            @error('address') 
                            <div class="alert alert-danger">{{ $message }}</div> 
                            @enderror <div class="bor8 m-b-30"> 
                                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" value="{{$users->address}}" type="text" name="address" placeholder="Your address"> 
                            </div> 
                            <div class="bor8 m-b-30"> 
                                <input type="radio" name="sex" style="margin-left: 60px;" value="1" {{$users->sex==1?'checked':''}}>
                                <label >Nam</label> 
                                <input type="radio" name="sex" style="margin-left: 60px;" value="0" {{$users->sex==0?'checked':''}}>
                                <label >Nữ</label> </div>  
                                <input type="hidden" name="id_role"  value="2"> <div class="m-b-30"> </div> 
                                <button class="flex-c-m stext-101 cl0 size-121 bg3 bor1 hov-btn3 p-lr-15 trans-04 pointer"> cập nhật </button>
                                 @csrf
                                 </form> 
                                </div> 
                            </div> 
                            <div class="col-11 col-md-5 col-lg-4 m-lr-auto"> 
                                ...
                                <button type="button" class="btn btn-success"><a href="/user/password/{{$users->id}}" style="color:white;"><iconify-icon icon="zmdi:edit"></iconify-icon> Đổi mật khẩu</a>
                            </button> <button type="button" class="btn btn-danger"><a a onclick="return confirm('Bạn có chắc là muốn xoá tài khoản này không?')" href="/user/delete/{{$users->id}}" style="color:white;"><iconify-icon icon="zmdi:delete"></iconify-icon> Xoá tài khoản</a>
